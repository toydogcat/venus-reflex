...
def process_volume(vol_name):
    ...
    print(f"[{vol_name}] Starting removal of page 2 (duplicate cover). Current page count: {page_count}")
    
    # 1. Read page 2 to find the image file to delete
    p2_json_path = pages_dir / '0002.json'
    if not p2_json_path.exists():
        print(f"  [{vol_name}] ERROR: pages/0002.json not found!")
        return False
    ...
    img_to_delete = None
    for el in p2_data.get('elements', []):
    ...
    if img_to_delete:
        ...
        if img_path.exists():
            img_path.unlink()
            print(f"  [{vol_name}] Deleted duplicate cover image: {img_to_delete}")
        else:
            print(f"  [{vol_name}] Warning: Image file {img_path} not found to delete.")
            
    # 2. Shift subsequent pages 3 to page_count down by 1
    # We do this in ascending order (3, 4, ..., page_count)
    for idx in range(3, page_count + 1):
